...
- ✅ **Fase 0 concluída de fato**: testado via `POST /consultas/nfe` do próprio serviço (não mais o `poc_consulta.py`) — resposta `{"c_stat": "138", "x_motivo": "Documento localizado"}`. Falta só validar o `docZip` (conteúdo do XML completo da nota).
